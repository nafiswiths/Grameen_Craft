...
export default function AdminDashboardView({
  ...
  const handleRoleChange = (email, newRole) => {
    ...
    // 3. Update PostgreSQL
    adminApi.changeUserRole(email, newRole)
      .then(data => {
        if (data.success) {
          triggerToast(
            language === "en" 
  ...
  const handleToggleStock = (prodId, currentStock) => {
    ...
    // 2. PostgreSQL
    adminApi.toggleProductStock(prodId, nextStockState)
      .then(data => {
        if (data.success) {
          triggerToast(
            language === "en" 
  ...
  const handleOrderStatusChange = (orderId, nextStatus) => {
    ...
    // 2. PostgreSQL
    adminApi.changeOrderStatus(orderId, nextStatus)
      .then(data => {
        if (data.success) {
          triggerToast(
            language === "en" 
