...
import { authApi, productApi, artisanApi, orderApi, dbApi, aiApi, adminApi } from "./api";
...
export default function App() {
  ...
  // Sync route path changes with custom view state
  useEffect(() => {
  ...
  // Sync custom view state changes with route path
  useEffect(() => {
    if (view && location.pathname !== `/${view}`) {
      navigate(`/${view}`);
    }
  }, [view]);
  ...
  const [registerDistrict, setRegisterDistrict] = useState("Jamalpur");
  ...
  const [bkashStep, setBkashStep] = useState("number");
  const [bkashNumber, setBkashNumber] = useState("");
  const [bkashOtp, setBkashOtp] = useState("");
  const [bkashInputOtp, setBkashInputOtp] = useState("");
  const [bkashPin, setBkashPin] = useState("");
  const [bkashAgreement, setBkashAgreement] = useState(true);
  const [bkashProcessingText, setBkashProcessingText] = useState("");
  ...
  const handlePlaceOrder = (e) => {
    ...
    if (paymentMethod === "bkash") {
      const newOrderTemp = {
        ...
        productName: cart[0]?.product[`name${language === "en" ? "En" : "Bn"}`] || "Village Crafts",
        paymentMethod: "bkash",
        bkashNumber: "",
        bkashTxnId: ""
      };
